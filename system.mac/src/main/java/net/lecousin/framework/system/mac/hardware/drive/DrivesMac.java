...
public class DrivesMac extends Drives {
	...
	private void newDisk(DADiskRef disk, Mutable<List<Pair<String, String>>> bsdNamesMountPoints, Mutable<List<DiskImageInfo>> diskImages) {
		...
		if (ptr != null && new CFBooleanRef(ptr).booleanValue()) {
			...
			PhysicalDriveMac drive = new PhysicalDriveMac();
		...
		synchronized (drives) {
			for (Drive d : drives)
				if (((PhysicalDriveMac)d).osId != null && ((PhysicalDriveMac)d).osId.equals(osId)) {
					part.drive = d;
					break;
				}
		}
		...
		for (DiskPartition p : ((PhysicalDriveMac)part.drive).partitions) {
			if (p.OSID.equals(bsdName))
				return;
		}
		((PhysicalDriveMac)part.drive).partitions.add(part);
		newPartition(part);
	}
	...
	private void diskRemoved(DADiskRef disk) {
		...
		if (ptr != null && new CFBooleanRef(ptr).booleanValue()) {
			...
			PhysicalDriveMac drive = null;
			synchronized (drives) {
				for (Iterator<Drive> it = drives.iterator(); it.hasNext(); ) {
					PhysicalDriveMac d = (PhysicalDriveMac)it.next();
		...
		synchronized (drives) {
			for (Drive d : drives)
				for (Iterator<DiskPartition> it = ((PhysicalDriveMac)d).partitions.iterator(); it.hasNext(); ) {
	...
	private void newDrive(PhysicalDriveMac drive) {
		...
		synchronized (drives) {
			for (Drive d : drives)
				if ((d instanceof PhysicalDriveMac) && ((PhysicalDriveMac)d).devpath.equals(drive.devpath)) {
	...
	private void driveRemoved(PhysicalDriveMac drive) {
		...
		for (DiskImageInfo di : diskImages.get()) {
			...
			for (String dev : di.devices)
				if (dev.equals(bsdName)) {
					...
					synchronized (drives) {
						for (Drive d : drives) {
							for (DiskPartition p : ((PhysicalDriveMac)d).partitions)
